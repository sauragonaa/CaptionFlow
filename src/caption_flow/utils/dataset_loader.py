...
class DatasetLoader:
    ...
    def _get_hf_shards(self) -> List[str]:
        ...
        files = [
            fs.resolve_path(p)
            for p in fs.glob(f"hf://datasets/{self.dataset_path}/**/*.tar")
        ]
        
        urls = [
            hf_hub_url(f.repo_id, f.path_in_repo, repo_type="dataset")
            for f in files
        ]
    ...
    def load_shard(self, shard_url: str, 
                  processed_keys: Optional[set] = None) -> wds.DataPipeline:
        """
    ...
    def iterate_shard(self, shard_url: str, 
                     processed_keys: Optional[set] = None) -> Generator[Tuple[str, str, bytes], None, None]:
        """
    ...
    def count_shard_items(self, shard_url: str, 
                         processed_keys: Optional[set] = None) -> int:
...
class ShardTracker:
    ...
    def update_partial(self, shard_name: str, processed_keys: List[str]):
        ...
        self.partial_shards[shard_name] = {
            "keys": processed_keys,
    ...
    def get_remaining_shards(self, all_shards: List[str]) -> List[str]:
        ...
        return [
            ...
            if Path(s).stem not in self.completed_shards
        ]