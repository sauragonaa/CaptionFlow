...
@click.pass_context
def orchestrator(ctx, port: int, host: str, data_dir: str, 
                cert: Optional[str], key: Optional[str], no_ssl: bool, vllm: bool):
    ...
    if not no_ssl:
        if cert and key:
            ...
            console.print("[yellow]Warning: Running without SSL. Use --cert and --key for production.[/yellow]")
    
    if vllm:
        # Use vLLM orchestrator with dataset support
        from .orchestrator_vllm import VLLMOrchestrator
        orchestrator = VLLMOrchestrator(config)
    else:
...
@click.pass_context
def worker(ctx, server: str, token: str, name: Optional[str], 
          batch_size: int, no_verify_ssl: bool, vllm: bool,
          gpu_id: int, precision: str, model: str):
    """Start a worker node."""
...
def generate_cert(domain: Optional[str], email: Optional[str], 
                 self_signed: bool, output_dir: str, staging: bool):
    ...
    if self_signed:
        ...
        console.print(f"[yellow]Requesting Let's Encrypt {mode} certificate for {domain}...[/yellow]")
        ...
        try:
            ...
            if staging:
                ...
                console.print("[yellow]  Remove --staging flag for production certificates[/yellow]")
...
def inspect_cert(cert_path: str):
    ...
    try:
        ...
        if info['not_after'] < datetime.utcnow():
            ...
            console.print(f"[yellow]⚠ Certificate expires in {(info['not_after'] - datetime.utcnow()).days} days[/yellow]")
        else:
            console.print(f"[green]✓ Certificate valid for {(info['not_after'] - datetime.utcnow()).days} more days[/green]")
